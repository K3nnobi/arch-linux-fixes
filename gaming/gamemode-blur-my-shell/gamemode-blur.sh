#!/usr/bin/env bash
# Runtime GameMode hook: temporarily disable selected GNOME extensions.
...
readonly EXTENSIONS_FILE="${ARCH_LINUX_FIXES_GAMEMODE_EXTENSIONS_FILE:-$CONFIG_HOME/arch-linux-fixes/gamemode-blur/extensions.conf}"
readonly RUNTIME_BASE="${XDG_RUNTIME_DIR:-/run/user/$(id -u)}"
readonly STATE_DIR="$RUNTIME_BASE/arch-linux-fixes-gamemode-blur"
readonly STATE_FILE="$STATE_DIR/extensions-were-enabled"
readonly LOCK_FILE="$STATE_DIR/lock"

log() {
    if [[ "${ARCH_LINUX_FIXES_GAMEMODE_QUIET:-0}" != '1' ]]; then
        printf 'Arch Linux Fixes GameMode: %s\n' "$*" >&2
    fi
}
...
hook_start() {
    ...
    # Duplicate GameMode start calls must not erase the restoration state.
    ...
    installed="$(installed_extensions)" || {
        log 'could not query installed GNOME extensions'
        return 1
    }
...
hook_end() {
    ...
    installed="$(installed_extensions)" || {
        log 'could not query installed GNOME extensions; restoration is pending'
        return 1
    }
